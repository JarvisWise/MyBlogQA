...
<div class="container-fluid text-center">
    <div class="row content my-c">
        <div class="col-xs-8 col-xs-offset-2 text-left my">
            <!-- Post List -->
            <c:forEach var="c" items="${postSets}">
                <div class="well">
                    <table class="table table-bordered tb-my cp" >
                        <tbody>
                        <tr>
                            <th class="alert alert-info">${c.getPost().getPostName()}</th>
                            <td class="alert alert-info">(Posted by: ${c.getPostUser().getFullName()})</td>
                            <td class="alert alert-info">(Posted: ${c.getPost().getPostDateTime()})</td>
                            <td class="alert alert-info">
                                <a href="<c:url value='/show/post?postId=${c.getPost().getPostId()}' />" class="btn btn-default">Show more...</a>
                            </td>
                        </tr>
                        <tr>
                            <td class="bg-success" colspan="4">${c.getPost().getPostText()}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
